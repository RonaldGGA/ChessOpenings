...
const convertToSan = (uciMove: string, fen: string, isPonder: boolean = false, bestMoveUci?: string): string => {
  try {
  ...
  const convertUCISequenceToSAN = (uciSequence: string, startFen: string): string => {
    try {
  ...
  const parseEvaluation = (evalStr: string, mateStr: string | null): { value: number; type: 'cp' | 'mate' } => {
    if (mateStr) {
  ...
  const getEvaluationPercentage = (evaluation: { value: number; type: 'cp' | 'mate' }): number => {
  ...
  const formatEvaluation = (evaluation: { value: number; type: 'cp' | 'mate' }): string => {
    if (evaluation.type === 'mate') {
  ...
  // Get advantage text
  const getAdvantageText = (evaluation: { value: number; type: 'cp' | 'mate' }): string => {
    if (evaluation.type === 'mate') {
  ...
  const evaluation = moveAnalysis ? parseEvaluation(moveAnalysis.evaluation, moveAnalysis.mate) : null;
  const evaluationPercentage = evaluation ? getEvaluationPercentage(evaluation) : 50;
  const advantageText = evaluation ? getAdvantageText(evaluation) : "No evaluation";
    ...
    <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-700 h-full">
      ...
      <div className="bg-slate-700/30 rounded-xl p-3 mb-4 border border-slate-600">
        <div className="flex justify-between items-center text-sm">
          ...
          <span className="text-yellow-400 font-mono">Depth {analysisDepth}</span>
        </div>
      </div>
        ...
        <div className="bg-slate-700/30 rounded-xl p-4 border border-slate-600 mb-4">
          <div className="flex justify-between items-center mb-3">
            ...
            <span className={`text-sm font-mono ${
              evaluation.value > 50 ? 'text-green-400' : 
              evaluation.value < -50 ? 'text-red-400' : 'text-yellow-400'
            }`}>
          ...
          <>
            ...
            <div className="bg-slate-700/30 rounded-xl p-4 border border-slate-600">
              ...
              <p className="text-white font-mono text-lg">
                {currentFen ? convertToSan(moveAnalysis.bestMove, currentFen) : moveAnalysis.bestMove}
              </p>
            </div>
            ...
            {moveAnalysis.continuation && (
              <div className="bg-slate-700/30 rounded-xl p-4 border border-slate-600">
                <h4 className="font-semibold text-yellow-400 text-sm mb-2">Suggested Line</h4>
                <div className="text-white text-sm font-mono">
                  {currentFen ? convertUCISequenceToSAN(moveAnalysis.continuation, currentFen) : moveAnalysis.continuation}
                </div>
              </div>
            )}
            ...
            {moveAnalysis.ponder && (
              <div className="bg-slate-700/30 rounded-xl p-4 border border-slate-600">
                <h4 className="font-semibold text-yellow-400 text-sm mb-2">Expected Response</h4>
                <p className="text-white font-mono text-sm">
                  {currentFen ? convertToSan(moveAnalysis.ponder, currentFen, true, moveAnalysis.bestMove) : moveAnalysis.ponder}
                </p>
              </div>
            )}
            ...
            {moveAnalysis.mate && (
              <div className="bg-slate-700/30 rounded-xl p-4 border border-slate-600">
                <h4 className="font-semibold text-yellow-400 text-sm mb-2">Game State</h4>
                <p className={`text-white font-mono text-lg ${
                  parseInt(moveAnalysis.mate) > 0 ? 'text-green-400' : 'text-red-400'
                }`}>
                  {parseInt(moveAnalysis.mate) > 0 
                    ? `White wins in ${moveAnalysis.mate} moves` 
                    : `Black wins in ${Math.abs(parseInt(moveAnalysis.mate))} moves`
                  }
                </p>
              </div>
            )}
